...
| Trend-with-regime-filter (Family A) finite-state trend rule: crossover begins an armed cycle, confirmation may occur on a later candle, no entry on an unconfirmed crossover, only one entry per bullish cycle, a bearish crossover resets the cycle, no lookahead | `test_research_candidates.py::test_trend_regime_no_entry_on_an_unconfirmed_crossover`, `::test_trend_regime_confirmation_can_occur_several_candles_after_the_crossover`, `::test_trend_regime_only_one_entry_per_cycle_even_across_a_forced_flat`, `::test_trend_regime_bearish_crossover_resets_the_cycle_for_a_fresh_entry`, `::test_trend_regime_no_lookahead` |
